...
#define _BSD_SOURCE
#define _GNU_SOURCE        // 更好的兼容性

#include <stdio.h>      // io perror
...
struct editorConfig
{
    ...
    int coloff;                     // 列偏移
    int rx;                         // 渲染光标位置
    struct termios orig_termios;    // 终端接口
    int numrows;                    // 行数
    struct editorRow *row;          // 行文本
    char *filename;                 // 文件名
    char statusmsg[80];             // 状态栏信息
    time_t statusmsg_time;          // 状态栏信息显示时间
    bool dirty;                     // 文件是否被修改
    struct editorSyntax *syntax;    // 文件类型
};
...
/*****************************************************************************/
// 字符串附加到动态buffer
static void abufAppend(struct abuf *ab, const char *s, int len)
{
...
// 析构函数
static void abufFree(struct abuf *ab)
{
    free(ab->b);
}
...
// 关闭回显，关闭规范模式，关闭信号，关闭输出处理...
static void enableRawMode()
{
    if(tcgetattr(STDIN_FILENO, &E.orig_termios) == -1)
        handle_error("tcgetattr");
    // 注册退出函数，确保程序退出时保留终端属性的原样
    ...
    // ECHO：启用输入字符的本地回显功能
    // ICANON：启用标准模式（行缓冲）
    // ISIG：Ctrl-C和Ctrl-Z发送信号
    // IEXTEN：Ctrl-v发送特殊字符
    raw.c_lflag &= ~(ECHO | ICANON | ISIG | IEXTEN);

    // IXON：启用软件流控制（XON/XOFF），Ctrl-s和Ctrl-q
    // ICNRL：将Ctrl-m转换为换行符
    // BRKINT：输入行检测到终止状态会发送SIGINT信号
    // INPCK：启用输入奇偶校验
    // ISTRIP：将输入字符的第8位设置为0
    ...
    // VMIN：最小输入字符数，如果为0，read()函数将立即返回
    // VTIME：读取字符的超时时间，单位为0.1秒
    raw.c_cc[VMIN] = 0;
    raw.c_cc[VTIME] = 1;

    // TCSAFLUSH：改变将立即发生，但不会立即生效，直到所有输出都被传输到终端
...
static void editorUpdateSyntax(struct editorRow *row)
{
...
/* 行操作 */
/*****************************************************************************/
static int editorRowCxToRx(struct editorRow *row, int cx)
{
    int rx = 0;
    for(int j = 0; j < cx; j++)
    {
...
static int editorRowRxToCx(struct editorRow *row, int rx)
{
    int cur_rx = 0;
    int cx;
...
static void editorUpdateRow(struct editorRow *row)
{
    int tabs = 0;
...
static void editorFreeRow(struct editorRow *row)
{
    free(row->render);
    free(row->chars);
    free(row->hl);
...
static void editorRowAppendString(struct editorRow *row, char *s, size_t len)
{
...
static void editorRowDeleteChar(struct editorRow *row, int at)
{
    if(at < 0 || at >= row->size)
        return;
...
/* 文件IO */
/*****************************************************************************/
static char *editorRowToString(int *buflen)
{
    int totlen = 0;
...
static int editorReadKey()
{
    ...
    while ((nread = read(STDIN_FILENO, &c, 1)) != 1)
        if (nread == -1 && errno != EAGAIN) handle_error("read");
            ...
            if (seq[1] >= '0' && seq[1] <= '9') {
                ...
                if (seq[2] == '~') {
                    switch (seq[1]) {
                        ...
                        case '6': return PAGE_DOWN; // PageUp, PageDown
                        case '1': return HOME_KEY; 
                        case '4': return END_KEY;
                        case '7': return HOME_KEY;
                        case '8': return END_KEY; // Home, End
                ...
                switch (seq[1]) {
                    ...
                    case 'D': return ARROW_LEFT; // 方向键
                    case 'H': return HOME_KEY;
                    case 'F': return END_KEY; // Home, End
...
void editorMoveCursor(int key)
{
    ...
    switch (key) {
        case ARROW_LEFT:
            if (E.cx != 0)
                E.cx--;
            else if (E.cy > 0) { // 下一行第一个字符位置左移到上一行最后一个字符位置
                E.cy--;
        ...
        case ARROW_RIGHT:
            if (row && E.cx < row->size)
                E.cx++;
            else if (row && E.cx == row->size) { // 上一行最后一个字符位置右移到下一行第一个字符位置
                E.cy++;
                E.cx = 0;
            }
            break;
...
{
    static int quit_times = TMUSH_QUIT_TIMES; // 多次确认才能不保存退出
...
static int getWindowSize(int *rows, int *cols)
{
    ...
    if(ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
        // 将光标移动到右下角，C B分别表示向右和向下移动，阻止光标越过屏幕边界
...
static void editorDrawRows(struct abuf *ab)
{
    ...
    for (int y = 0; y < E.screenrows; y++) {
        ...
        if (filerow >= E.numrows) {
            ...
            if (y == E.screenrows / 3 && E.numrows == 0) {
                ...
                int welcomelen = snprintf(welcome, sizeof(welcome),
                    "Tmush editor -- version %s", VERSION);
        ...
        abufAppend(ab, "\x1b[K", 3); // 清除从光标到行尾的内容
        abufAppend(ab, "\r\n", 2);
    }
}
...
static void editorRefreshScreen()
{
    ...
    abufAppend(&ab, "\x1b[?25l", 6);    // 隐藏光标
    abufAppend(&ab, "\x1b[H", 3);       // 将光标移动到左上角
...
static void initEditor()
{
    ...
    if (getWindowSize(&E.screenrows, &E.screencols) == -1)
        handle_error("getWindowSize");
    E.screenrows -= 2; // 用作状态栏
}
